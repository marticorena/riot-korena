...
class RiotClient:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                try:
                    ...
                    if response.status_code == 429:
                        ...
                        logger.warning(f"Riot 429 Rate Limit directly triggered. Halting for {retry_after}s.")
                    ...
                    logger.error(f"HTTP error {exc.response.status_code} mapped from {url}")
                    return None
    ...
    async def get_match_ids_by_puuid(self, puuid: str, region: str, count: int = 5) -> Optional[list[str]]:
        routing = self._get_regional_routing(region)
        url = f"https://{routing}.api.riotgames.com/lol/match/v5/matches/by-puuid/{puuid}/ids?count={count}"
        return await self._get(url, routing)

    async def get_match_by_id(self, match_id: str, region: str) -> Optional[Dict[str, Any]]:
        routing = self._get_regional_routing(region)
        url = f"https://{routing}.api.riotgames.com/lol/match/v5/matches/{match_id}"
        return await self._get(url, routing)

    async def get_match_timeline_by_id(self, match_id: str, region: str) -> Optional[Dict[str, Any]]:
        routing = self._get_regional_routing(region)
